...
// --- TYPES ---
interface MarketCoin {
  symbol: string; name: string; price_usd: number; market_cap: number; total_volume: number; change_24h_pct: number; drawdown_pct: number; logo_url: string; rank: number;
}
interface HistoricalData {
  date: string; price_usd: number; moving_avg_7d: number; total_volume: number;
}
...
const CryptoDashboard: React.FC = () => {
  ...
  // --- API URL (FINAL FIX) ---
  // Using the LIVE URL as the primary source to bypass Vercel environment variable issues
  const LIVE_RENDER_URL = 'https://meresa.onrender.com';
  const API_BASE_URL = LIVE_RENDER_URL;
