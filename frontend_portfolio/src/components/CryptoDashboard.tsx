...
const apiAxios = axios.create({
  // Increased timeout for Render cold start on slow connections
  timeout: 2000000
});
...
const CryptoDashboard: React.FC = () => {
  ...
  const handleDrillDown = async (symbol: string) => {
    ...
    try {
      ...
      const res = await axios.get(`${API_BASE_URL}/api/crypto-trends?symbol=${coin.symbol}`);
      setHistory(res.data);
    } catch (err) {
      console.error(err);
    }
  ...
  const pieData = useMemo(() => { /* ... (Logic remains the same) ... */
  ...
  const treeMapData = useMemo(() => { /* ... (Logic remains the same) ... */
    return [{
      name: 'Market',
  ...
  const globalStats = useMemo(() => { /* ... (Logic remains the same) ... */
    ...
    <div className="w-full bg-[#0b0e11] min-h-[800px] p-6 rounded-2xl border border-gray-800 shadow-2xl text-white font-sans">
      ...
      <div className="flex justify-between items-center mb-6 pb-4 border-b border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-1xl font-bold">Crypto Data Analytics & Visualization</h1>
        ...
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          {!loading && marketData.length > 0 && (
            <div className="grid grid-cols-1 md:grid-cols-1 lg:grid-cols-3 gap-6 mb-8 min-h-[300px] items-stretch">
              ...
              {/* 1. TREEMAP (Market Cap vs Change) */}
              <div className="lg:col-span-2 bg-[#15191f] p-4 rounded-xl border border-gray-800">
                ...
                <div style={{ width: '100%', height: '90%', minWidth: '0px', overflow: 'hidden' }}> {/* FIX: min-w-0 wrapper */}
              ...
              <div className="bg-[#15191f] p-4 rounded-xl border border-gray-800">
                ...
                {/* 🚀 FIX: The mobile width reset is critical here. */}
                    ...
                    <PieChart>
                      ...
                      >
                        {pieData.map((entry, index) => (
                          // ADDED Click handler for drill-down
                          <Cell
                            key={`cell-${index}`}
                      ...
                      {/* Legend moved below the PieChart component for better flow */}
